<?php
#Nome do arquivo: curtida/list.php
#Objetivo: interface para listagem de curtidas do usuário

require_once(__DIR__ . "/../include/header.php");
require_once(__DIR__ . "/../include/menu.php");
?>

<h3 class="text-center">Produtos curtidas</h3>

<div class="container">
    <div class="row" style="margin-top: 10px;">
        <div class="col-12">
            <table id="tabProdutos" class='table table-striped table-bordered'>
                <thead>
                    <tr>
                        <th>Nome</th>
                        <th>Descrição</th>
                        <th>Preço</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach($dados['lista'] as $c): ?>
                        <tr>
                            <td><a href="<?=BASEURL?>/controller/ProdutoController.php?action=display&id=<?=$c->getProduto()->getId()?>"><?= $c->getProduto()->getNome(); ?></a></td>
                            <td><?= $c->getProduto()->getDescricao(); ?></td>
                            <td><?= $c->getProduto()->getPreco(); ?></td>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
    </div>
</div>

<?php  
require_once(__DIR__ . "/../include/footer.php");
?>
